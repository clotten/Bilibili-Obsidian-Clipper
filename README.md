...
刷新批量页面或断点续跑时，已经存在的小合集教程会自动跳过，不会重复调用 AI 合成；只有缺失或写入失败的小合集才会重新生成。

批量生成过程中点击“暂停”会取消当前 AI 请求，并在当前步骤收尾后停止，不会继续处理下一个视频或小合集。

批量页面读取合集后默认不会自动开始。可以勾选要处理的小合集，点击“开始生成”；暂停后点击“开始/继续生成”即可从断点继续。

1.3.0 增加了 AI 分组建议和人工分组：AI 只根据视频标题返回连续区间建议，不会自动开始；可以修改小合集名称、添加小合集，并为每个视频选择所属小合集。视频笔记和小合集教程分开生成，小合集可单独生成或批量合成；合成时默认只使用单视频笔记，也可以手动勾选加入原始字幕和填写额外要求。
